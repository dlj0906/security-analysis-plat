    ...
    >
      <ElScrollbar style="height: calc(100% - 10px)">
        <div
          ...
          :style="{ background: getMenuTheme.background }"
        >
          <ArtLogo v-if="!isDualMenu" class="logo" size="180" />
        </div>
...
<script setup lang="ts">
  ...
  const firstLevelMenus = computed(() => {
    return useMenuStore().menuList.filter((menu: AppRouteRecord) => !menu.meta.isHide)
  })
  ...
  const menuList = computed(() => {
    ...
    const currentMenu = allMenus.find((menu: AppRouteRecord) => menu.path === currentTopPath)
